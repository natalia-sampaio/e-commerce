...
import { getMensShoes, getWomensShoes } from './services/products'
...
<template>
  <header
    class="flex items-center justify-between transition-all duration-500 xl:mx-40 xl:mb-20 xl:border-b-2 xl:border-blue-light-grayish"
  >
    <div class="flex items-center transition-all duration-500">
      <IconMenu
        class="m-3 transition-all duration-700 motion-reduce:transition-all xl:hidden"
      ...
      <div v-if="desktop" class="w-full">
        <ul class="my-10 text-blue-dark-grayish font-normal flex justify-evenly">
          <li class="my-5 ml-5 hover:font-bold hover:text-blue-very-dark p-2">
            <RouterLink :to="{ name: 'collections' }" @click="mobileNav = false"
          ...
          <li
            v-if="!cartStore.isLoggedIn"
            class="my-5 ml-5 hover:font-bold hover:text-blue-very-dark p-2"
          >
            <RouterLink :to="{ name: 'sign-in' }">Sign in</RouterLink>
          </li>
          <li
            v-if="!cartStore.isLoggedIn"
            class="my-5 ml-5 hover:font-bold hover:text-blue-very-dark border border-blue-grayish hover:border-black rounded p-2"
          >
      ...
      <Transition name="mobile-nav">
        <div
          v-show="mobileNav"
          class="bg-white fixed xl:relative top-0 left-0 h-full xl:h-auto w-2/3 xl:w-full z-30 xl:z-0"
        >
          <IconClose
            class="m-5 absolute right-0 transition-all duration-700 motion-reduce:transition-all xl:hidden"
            @click="toggleMobileNav"
            :class="{ '-rotate-180': !mobileNav }"
            :fill="'#69707D'"
          />
          <ul class="mx-5 my-10 xl:text-blue-dark-grayish xl:font-normal xl:flex xl:justify-evenly">
            ...
            <li
              v-if="!cartStore.isLoggedIn"
              class="my-5 xl:ml-10 border-b border-blue-dark-grayish pb-5"
            >
              <RouterLink :to="{ name: 'sign-in' }">Sign in</RouterLink>
            </li>
    ...
    <div class="flex items-center">
      <IconHeart
        ...
        stroke="#69707D"
        class="w-6 h-6"
        @click="favoritesStore.toggleFavorites = !favoritesStore.toggleFavorites"
      />
      <button class="relative">
        <span class="text-xs text-white rounded-3xl px-2 bg-orange-main absolute right-0 top-1">{{
          cartStore.cartItems
        }}</span>
        <IconCart
          :fill="cartStore.toggleCart ? '#000000' : '#69707D'"
          class="m-3"
      ...
      <RouterLink :to="{ name: 'user-profile' }">
        <div class="w-8 h-8 m-3 xl:w-16 xl:h-16 xl:ml-10">
          <img
            ...
            alt=""
            class="w-8 h-8 xl:w-16 xl:h-16 rounded-full object-cover"
          />
  ...
  <RouterView v-slot="{ Component }">
    <template v-if="Component">
      <SlideDownFade>
        <Suspense>
          <component :is="Component" :isLoggedIn="cartStore.isLoggedIn" />
